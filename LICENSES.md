# Licensing and attribution

This repository is a collection of independently sourced skills. It does not
apply one blanket license to every directory.

- Pstack-derived skills are imported from
  [`michael-denyer/pstack-claude`](https://github.com/michael-denyer/pstack-claude)
  under the MIT license. The complete license and upstream notices are in
  [`skills/poteto-mode/references/licenses/`](skills/poteto-mode/references/licenses/).
- `before-and-after` carries its PolyForm Shield 1.0.0 license in
  [`skills/before-and-after/LICENSE`](skills/before-and-after/LICENSE).
- `greploop` and `greploop-apps` carry the MIT license in their skill
  directories.
- `unslop` carries its MIT license in
  [`skills/unslop/LICENSE`](skills/unslop/LICENSE).
- Skills originating in
  [`michaelshimeles/skills`](https://github.com/michaelshimeles/skills) retain
  their source history and any license included with the source skill.

Files without an explicit license notice remain subject to their respective
copyright holders' rights. Consult the named source and bundled license before
redistributing an individual skill outside this collection.

The integration metadata, installer, vendor tooling, and Mission Control
compatibility changes do not replace or broaden an upstream license.
