...
3. Explore in subagents with the plugin type `subagent_type: "software-factory-skills:poteto-agent"`, or bare `poteto-agent` for a direct install, and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
