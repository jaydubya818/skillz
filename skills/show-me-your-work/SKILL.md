...
description: "Use when long-running work needs an auditable decision and evidence log."
license: MIT
metadata:
...
At the end of the run, before handing back, check the log told the truth. On Claude Code, read this run's transcript under its per-project transcript directory at `~/.claude/projects/<encoded-cwd>/`. On Codex or another harness, use its current-task history API or documented workspace-scoped transcript location. Don't glob across `~/.claude/projects/`; that reads unrelated private chats. Walk the log against what actually happened:
